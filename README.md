...
Material 3 starter — NavigationBar, search, detail route, widget tests.

**Live demo** — https://flutter.hanzo.app — the **same** app, compiled to WebAssembly by `flutter build web`. Not a mock-up.

## What you get

`NavigationBar` over an `IndexedStack` so each tab keeps its own scroll
position, a `SliverAppBar.large` feed, a `SearchBar` filter and two widget tests
that actually drive the UI.
